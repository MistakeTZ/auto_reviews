      ...
      case 0:
        return (
          <div className="w-full max-w-[440px] overflow-hidden rounded-[12px] border border-black/5 bg-white shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05),0_8px_10px_-6px_rgba(0,0,0,0.03)] transition duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]">
            <div className="flex items-center gap-1 border-b border-[#f0f0f0] bg-[#fafafa] px-4 py-3.5">
            ...
            <div className="space-y-4 p-6">
              <div>
                ...
                <div className="mt-1.5 flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 rounded-md border border-[#edf2f7] bg-[#f7fafc] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4a5568]">
                    Xiaomi <button className="leading-none text-[#cbd5e0]">×</button>
                  </span>
                  <span className="inline-flex items-center gap-1 rounded-md border border-[#edf2f7] bg-[#f7fafc] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4a5568]">
                    Apple <button className="leading-none text-[#cbd5e0]">×</button>
                  </span>
                  <span className="inline-flex cursor-pointer items-center rounded-md border border-dashed border-[#e2e8f0] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4f46e5] transition-colors hover:bg-[#fafafa]">+ {language === 'en' ? 'Add brand' : 'Добавить бренд'}</span>
                </div>
              </div>
              <div>
                ...
                <div className="mt-1.5 flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 rounded-md border border-[#edf2f7] bg-[#f7fafc] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4a5568]">
                    {language === 'en' ? 'Phone cases' : 'Чехлы для телефонов'} <button className="leading-none text-[#cbd5e0]">×</button>
                  </span>
                  <span className="inline-flex cursor-pointer items-center rounded-md border border-dashed border-[#e2e8f0] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4f46e5] transition-colors hover:bg-[#fafafa]">+ {language === 'en' ? 'Add category' : 'Добавить предмет'}</span>
                </div>
              </div>
              <div>
                <label className="text-[0.8rem] font-bold text-[#0A192F]">{language === 'en' ? 'Articles (SKU):' : 'Артикулы (SKU):'}</label>
                <div className="mt-1.5 flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 rounded-md border border-[#edf2f7] bg-[#f7fafc] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4a5568]">19284812 <button className="leading-none text-[#cbd5e0]">×</button></span>
                  <span className="inline-flex items-center gap-1 rounded-md border border-[#edf2f7] bg-[#f7fafc] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4a5568]">38472910 <button className="leading-none text-[#cbd5e0]">×</button></span>
                  <span className="inline-flex cursor-pointer items-center rounded-md border border-dashed border-[#e2e8f0] px-2.5 py-1 text-[0.75rem] font-semibold text-[#4f46e5] transition-colors hover:bg-[#fafafa]">+ {language === 'en' ? 'Add SKU' : 'Добавить артикул'}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3 p-6">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {['брак', 'порван', 'возврат', 'подделка'].map((word, idx) => (
                  <span key={idx} className="inline-flex items-center gap-1 rounded-md border border-red-100 bg-red-50 px-2.5 py-1 text-[0.75rem] font-semibold text-red-700">
                    {language === 'en' ? (word === 'брак' ? 'defect' : word === 'порван' ? 'torn' : word === 'возврат' ? 'return' : 'fake') : word}
                    <button className="leading-none text-red-400 hover:text-red-600">×</button>
                  </span>
                ))}
                <span className="inline-flex cursor-pointer items-center rounded-md border border-dashed border-red-200 px-2.5 py-1 text-[0.75rem] font-semibold text-red-500 transition-colors hover:bg-red-50/50">+ {language === 'en' ? 'Add stop-word' : 'Добавить слово'}</span>
              </div>
            </div>
          </div>
        );
            ...
            <div className="space-y-3 p-6">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {['подарок', 'качество', 'быстро', 'спасибо'].map((word, idx) => (
                  <span key={idx} className="inline-flex items-center gap-1 rounded-md border border-green-100 bg-green-50 px-2.5 py-1 text-[0.75rem] font-semibold text-green-700">
                    {language === 'en' ? (word === 'подарок' ? 'gift' : word === 'качество' ? 'quality' : word === 'быстро' ? 'fast' : 'thanks') : word}
                    <button className="leading-none text-green-400 hover:text-green-600">×</button>
                  </span>
                ))}
                <span className="inline-flex cursor-pointer items-center rounded-md border border-dashed border-green-200 px-2.5 py-1 text-[0.75rem] font-semibold text-green-500 transition-colors hover:bg-green-50/50">+ {language === 'en' ? 'Add trigger word' : 'Добавить слово'}</span>
              </div>
            </div>
          </div>
        );
  ...
  return (
    <section className="overflow-hidden bg-[linear-gradient(180deg,#ffffff_0%,#f6f8fb_100%)] px-4 py-16 lg:px-8 lg:py-20" style={{ contentVisibility: 'auto', containIntrinsicSize: '850px' }}>
      <div className="relative mx-auto max-w-[1200px]">
        ...
        <div className="mb-14 w-full overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" data-reveal="up">
          <div className="mx-auto flex min-w-max justify-center gap-3 px-1">
            {scenariosTabs.map((tab) => (
              <button
                key={tab.id}
                className={`rounded-full border px-5 py-2.5 text-sm font-semibold whitespace-nowrap transition-colors duration-200 ${activeScenarioTab === tab.id ? 'border-indigo-400 bg-indigo-50 text-indigo-600 shadow-[0_2px_8px_rgba(99,102,241,0.08)]' : 'border-slate-200 bg-white text-slate-500 hover:border-slate-300 hover:bg-slate-50 hover:text-[#0A192F]'}`}
        ...
        <div className="grid items-center gap-12 xl:grid-cols-[1.2fr_1fr]">
          <div className="w-full" data-reveal="zoom" style={revealDelay(120)}>
            <div className="flex items-center justify-center rounded-[24px] bg-[#f3f4f6] p-6 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05),0_10px_15px_-3px_rgba(0,0,0,0.02)] sm:p-8 lg:p-12">
          ...
          <div className="hidden flex-col items-start lg:flex" data-reveal="right" style={revealDelay(180)}>
            <h3 className="mb-4 text-[clamp(1.5rem,2.5vw,2.2rem)] font-extrabold tracking-[-0.5px] text-[#0A192F]">{scenariosTabs[activeScenarioTab].title}</h3>
            <p className="mb-8 text-[clamp(0.95rem,1.6vw,1.1rem)] font-medium leading-[1.6] text-[#4A5568]">
              {t(`landing.scenariosTab${activeScenarioTab}Desc`)}
            </p>
