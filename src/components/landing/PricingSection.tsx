...
const PRICING_PLANS: {
  ...
  {
    titleKey: "landing.pricingCombo",
    priceKey: "landing.pricingComboPrice",
    descKey: "landing.pricingComboDesc",
    isPopular: false,
    isProfitable: true,
    periodKey: "landing.pricingComboPeriod",
    badgeKey: "landing.pricingComboBadge",
  },
];
    ...
    >
      ...
      <div className="pricing-grid mx-auto grid max-w-[1200px] gap-8 lg:grid-cols-3">
        {PRICING_PLANS.map((plan, idx) => (
          <Reveal
            key={idx}
            className={`pricing-card relative flex flex-col rounded-[24px] border p-10 transition duration-200 hover:-translate-y-1.5 ${
              plan.isPopular
